...
function acquireRtools() {
    return __awaiter(this, void 0, void 0, function* () {
        let fileName = "rtools.3.5.0.nupkg";
        try {
            let downloadPath = yield tc.downloadTool(util.format("https://github.com/hannesmuehleisen/choco-rtools/raw/master/%s", fileName));
...
function getDownloadUrlMacOS(version) {
    const filename = util.format("R-%s.pkg", version);
    if (semver.lt(version, "3.4")) { // older versions are in old
